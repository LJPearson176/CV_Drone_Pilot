...
export const useAppStore = (): Omit<AppContextType, keyof ReturnType<typeof useDummyHandlers>> => {
  ...
  // Kinematic Controls
  const [isKinematicEnabled, setIsKinematicEnabled] = useState<boolean>(false);
  const isKinematicEnabledRef = useRef(isKinematicEnabled); // Ref to avoid stale closure in gameLoop
  const kinematicLeftStickRef = useRef<[number, number]>([0, 0]);
  const kinematicRightStickRef = useRef<[number, number]>([0, 0]);
  ...
  useEffect(() => { isKinematicEnabledRef.current = isKinematicEnabled; }, [isKinematicEnabled]);
  ...
  useEffect(() => {
    ...
    const gameLoop = (timestamp: number) => {
        ...
        const isKinematic = isKinematicEnabledRef.current;

        // --- Physics & Camera ---
        const keys = keysPressed.current;
        
        // --- Kinematic Input ---
        // Retrieve joystick values (-1.0 to 1.0)
        const leftJoy = kinematicLeftStickRef.current; // [x (strafe), y (throttle)]
        const rightJoy = kinematicRightStickRef.current; // [x (yaw), y (pitch)]

        // Forward/Back
        let fwd = (keys.has('w') ? 1 : 0) - (keys.has('s') ? 1 : 0);
        
        // Apply Left Joystick Y (Throttle): Up (-Y) -> Forward (+Fwd)
        if (isKinematic) fwd += -leftJoy[1];

        if (controls.invertForward) fwd = -fwd;

        // Strafe Left/Right
        let str = (keys.has('d') ? 1 : 0) - (keys.has('a') ? 1 : 0);
        // Apply Left Joystick X (Strafe)
        if (isKinematic) str += leftJoy[0];
        ...
        if (isKinematic) {
             // Hand Up -> rightJoy[1] is negative. Looking Up is pitchInput negative.
             pitchInput += rightJoy[1];
        }
        ...
        // Apply Right Joystick X (Yaw)
        if (isKinematic) yawInput += rightJoy[0];
  ...
  return {
    ...
    isKinematicEnabled, setIsKinematicEnabled, kinematicLeftStickRef, kinematicRightStickRef
  };
};
